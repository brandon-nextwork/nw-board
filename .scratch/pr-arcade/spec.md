...
- **Ingress**: GitHub per-repo webhooks on the three Tracked Repos (`projects-app`, `features`, `content` in `nextworkengineering`), delivered to the Pi through Tailscale Funnel (free tier, stable `*.ts.net` hostname, TLS handled by Tailscale). Webhook payloads verified with the shared webhook secret.
